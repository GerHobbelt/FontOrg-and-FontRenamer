/*
  Compare Folders #3 - Compare Files, Folders, and Checksums
...
public class CompareFolders3
{
  ...
  static final String PROGRAM_TITLE =
    "Compare Files, Folders, and Checksums - by: Keith Fenske";
...
//      || word.equals("-h") || (mswinFlag && word.equals("/h")) // see: hidden
      ...
      else if (word.startsWith("-b") || (mswinFlag && word.startsWith("/b")))
      {
        ...
        60 MB allows us only two buffers of 16 MB each.  Nowhere do we catch
        OutOfMemoryError exceptions.  The only way to cause these errors is by
        the choice of buffer size on the command line, so letting Java print an
        error message on standard error below the command line is a reasonable
        warning to the user. */
...
*/
    ...
    File secondFile)              // second checksum file (only), or null
  {
    ...
    boolean askUser;              // while we are asking user what to do
    CompareFolders3File firstChecksum; // first calculated checksums
    ...
    if ((consoleFlag == false) && (secondFile == null)) // GUI output only
      outputText.setText("");     // clear output text area
    askUser = true;               // keep asking until we get an answer
    ...
    while (askUser)               // repeat because output file may be bad
    {
      ...
      if (cancelFlag == false)    // if file was written successfully
        askUser = false;          // stop asking user where to save file
      else if (consoleFlag)       // are we running from the command line?
        askUser = false;          // yes, don't try again, don't prompt user
      else if (userFile == null)  // are we writing to GUI output text area?
        askUser = false;          // yes, don't prompt user if GUI has failed
...
/*
  ...
  or folders, we always do a comparison.
...
*/
  static int doConsoleFiles(String firstName, String secondName)
  {
    ...
    /* If two file or folder names were given, then we always do a comparison.
    Files must contain checksum information in XML format. */
    ...
    else
    {
      ...
      {
        status = compareFolderChecksum(secondFile, firstFile);
...
*/
  static void doStartRunner()
  {
    ...
    switch(actionDialog.getSelectedIndex())
    {
      ...
      checksum file/folder, to find either duplicate files (that is, the same)
      or unique files (different).  There are actually six situations:
      ...
      All duplicate file methods scan recursively, independent of the value of
      the <recurseFlag> global variable.  Otherwise, would the flag apply to
      the first checksum/folder, the second checksum/folder, or both? */
    ...
    if (errorFlag)                // was there a problem with the user's input?
    {
      ...
      /* Show the user what we found for files and/or folders, in case this is
      not what the user was expecting, and hence is the cause of the error. */
...
*/
    ...
    CompareFolders3File givenEntry) // caller gives us one file checksum object
  {
    ...
    {
      setStatusMessage("Scanning " + pathPrefix + givenEntry.name);
...
*/
    ...
    File givenFile)               // caller gives us one file or folder
  {
    ...
    {
      setStatusMessage("Scanning " + givenFile.getPath());
      ...
      for (i = 0; i < contents.length; i ++) // for each file in order
      {
        ...
        if ((hiddenFlag == false) && next.isHidden()) // hidden file or folder?
          ...
          findDupScanFolder(sizeList, orderList, pathPrefix, next); // yes
        else
...
*/
    ...
    String source)                // original data source, or empty string
  {
    ...
    try
    {
      ...
      writeCheckSumLine(output,
        "<!-- http://www.psc-consulting.ca/fenske/cporder.htm -->");
